...
- `-reg` 位打包顺序 = 登记顺序（GUI 约定，板上回归实测）：**第一个登记的探针对应读值位串的 MSB 端切片**（切片内反转即为探针值）。RTL 连线时注意 JTAG 链方向与端口序号相反——probe_in[低位字节] 会出现在读值高位端。多个探针可用逗号分隔（`-reg -in cnt:8,fb:8`，3.13.2 起）或重复 `-in`/`-out`。
- **对位验证（R23 板上实测修正）**：MHz 级窄计数器 + 秒级软件采样是欠采样，相邻读数差不具等差性（采样抖动 1µs ≈ 25 count 漂移），不要拿"差值恒定"当判据。正确做法是**联合判定**：①各槽活性/分化行为符合物理连线（计数器槽持续变化、常量槽恒定）；②用 `-write` 驱动回环/已知信号读回核对（写非回文值如 0xC1 可同时验证位序，0xA5/0x3C 是回文不能）。判对位错误再交换登记顺序，无需重编译。
