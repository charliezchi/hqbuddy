...
- RTL 风格：端口用 ANSI 风格 `output reg` 后**不得**再单独 `reg` 声明同名信号（elaborate 报重复声明）
- 管脚：时钟必须用时钟能力引脚（N 侧 Tx 脚等会被 PHY-CHK-030 拒绝）；板载晶振管脚按板查
- **下载必须 `--Burst`**：压缩 bin 用非 Burst 模式 cable 会报
  `ERROR: Do not support compress bit file` 且看起来只是"没下载成功"
- **del→re-add 追加到组末**：不回填原槽位，采样组顺序随增删历史变化（存储打包随之
  变化）——这就是"改信号必须 -run+重下"的根本原因；未重跑前抓波形：通道数/触发都对
  但数值视图错位，极具迷惑性
- `-del` 被触发条件引用的信号时会提示悬空表达式，请随即重新 `-trig`
- 跨时钟域信号：确认其确实由现采样时钟驱动（同同步域）后可去掉 `-clk` 直接添加；若它属于别的时钟域，采样到的会是常数或无效值——这不是工具 bug。
- **RTL 源码修改后必须删 hqins_run 重建**：`-insight -run` 会复用旧的
  `hq_import_with_bscan.v`（零警告、零报错），板上是旧网表（R47d 实测）。
- **XiST 子集不支持 `initial` 块**：无复位网络的 FF 上电初值不确定。
- **插桩流程不读 SDC**：时序签核链在插桩位流上被绕过。
- **GUI/CLI 不要交叉持有同一 .hqins**：GUI 打开期间用 CLI 增删的信号，GUI 一保存就丢（GUI 以内存态覆盖磁盘）。GUI 添加/删除/改类型信号的真实入口是源码编辑器里右键信号 token（上下文菜单选类型），不在层次树。
...
- **触发标记点的取值有流水偏斜**：trigger_event 所在样本的原始值不一定逐字满足触发条件，且偏斜量因 bit 而异（实测 -1 ~ +8 拍都有）。判读触发点时取 trigger_event 前 8 拍到后 8 拍的窗口，以"窗口内出现满足条件的值"为准；跨 run 比较时注意偏斜方向可能不同。
- **多总线存储字打包位序**（多模块、位宽不一的多总线设计偶发）：插桩网表的
  存储字打包顺序可能与 ddf/信号清单不一致 → VCD 通道混杂、按字面值触发永不命中。
  症状自检：触发点数据不符合任何合理语义、且 AND 单条件也不命中。缓解：探针总线
  位宽对齐、或每个时钟域只用一条宽总线；发现后调整信号组合重跑 -run
- **触发单元运行时失效**（偶发）：某信号的任何触发条件都不命中（AND/单条件/边沿/算术全不命中），而该信号通道在波形里活跃翻转——按位布局的偶发问题（网表取证探针存在且翻转、条件文件正确）。处理：调整触发信号添加顺序或增删一个信号后重跑 -run 生成新布局；这也是"多准备一个触发信号"的价值。
...
## 波形判读补充：overflow 标志与触发点语义（R27→R31 修正）

`-capture` 输出 `(pointer=N, overflow=True)` 只说明布防→命中间隔超过一个缓冲深
（自由运行设计的常态），**不代表标记点不可判读**。R31 板上实证：三次抓取
pointer 各异（181/20/134）而标记点 #128 样本逐字相同且严格满足条件（随机命中
概率 ~6×10⁻⁸）——存储窗锚定在匹配样本上，标记点单样本判据依然严格有效。
R27 观察到的标记点值不满足条件的现象未在受控实验中复现，若再次遇到请先核对
布防条件与下载 bit 是否一致（信号集戳记警告）。

**混位宽探针 = 数据静默损坏（R25+R31 双重实证）**：8b+8b 干净（0 违例），
追加 1-bit 后 8b 通道转移违例 770/1023、1-bit 通道出现非物理波形——且流程
exit 0 无任何警告。hqbuddy 3.13.4 起 `-add` 检测到位宽不一致会主动警告；
**探针集合保持同位宽**，混位宽 bit 上的任何判读（含边沿方向）均无效。

## X 通配触发（R49 实测：FT091626 上不可用）

`-trig "cnt EQ xxxxx000"` 语法已实现（值含 x/X 的位串→通配，x 位写 ddf mask），
ddf mask 正确写入（如 11111000），但**板级触发行为与通配语义矛盾**：
触发点样本不满足掩码条件、VCD 中 trigger_event 全程为 0（与"已触发"输出矛盾）、
独立解析零匹配。**判定：S1 不可用，X 通配条件勿用**；保留语法等厂商修复。
（对照：普通 EQ/NE/GT/RANGE/跨信号 AND 在同板同版本上全部精确。）
