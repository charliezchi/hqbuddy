...
- `references/boards.md` — 开发板板卡资料查阅（`references/boards/` 下各开发板的 FPGA 引脚分配；做引脚约束、对接开发板外设时先读这个）
...
- `references/soc_workflow.md` — SoC（FPGA+ARM 软核）全 CLI 工作流（-new_soc/-build/-mcu_build/-merge_bin 链路、预设清单、工程结构，做 SA5Z-30/50 SoC 开发时必读）
...
- `references/thirdparty_synthesis.md` — 第三方综合（Synplify/Vivado → EDIF → HqFpga P&R → bin）全 CLI 链路与 EDF→Verilog 转换、第三方网表+VLA 调试边界（用户要求用第三方综合工具时必读）
- `references/hqinsight_gui_map.md` — HqInsight/VLA GUI 逆向笔记（.hqins 键值、vla.cfg 交接格式、SVF 生成链，做 insight 底层排障时按需读）
