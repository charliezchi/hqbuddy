...
- 09-17 07:05 R47c 静默回归：基线抓取 cnt=200 精确；-edf2v 冒烟 13×xsDFFSA。
  无异常，一句话收尾。

- 09-17 06:22 R47b 静默回归：基线抓取 cnt=200(0xc8) 严格、lfsr 满足条件。
  无异常，一句话收尾。

- 09-17 05:41 定时轮启动 R43b（depth=2048 端到端补完，干净状态）。**判据**：
  ①-depth 2048 → -run：成功则继续，崩溃则记录"2048 亦不可用"并回退 1024；
  ②成功时：下载 + -capture，样本数（期望 ~2048 时间戳）+ 触发点 cnt=200 严格
  + clock_cycle 13 位；③收尾恢复 1024 基线（-depth 1024 → -run → 下载 → 抓取
  验证 1024）。全程如实记录。

- 09-17 06:00 R43b 收账：**2048 端到端补测定论**——结构生效（2049 样本、
  cc 13 位）但触发语义失真（te 窗口 cnt≈129-134 ≠ EQ 200）→ depth≠1024
  触发不可靠，生产保持 1024（insight.md 已更新）；1024 基线复原验证。
  板上=r21a 干净 1024 bit。
