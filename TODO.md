...
- [x] 第三夜收官（2026-09-17 08:00）——R43-R51 共 12 轮；深度链路定论（保持
      1024）、-del 崩溃根因+防护、VLA IP 第三方综合验证可行；总账见
      `guides/night_report_20260917.md`
- [ ] 白天优先：4096 崩溃与 -del 空触发集段错误向智多晶反馈（素材均齐）；
      VLA 运行侧 ddf 生成链 GUI 抓包；VLA 运行侧逆向；边沿×混位宽交叉复测
...
- [x] R47：-del 段错误取证+防护（2026-09-17）——100% 复现→根因=删空触发集
      （与全 sample 红线同源）；-del 拒绝清空触发集+指引；厂商素材留存
      r47del/vendor_feedback/
- [ ] 小项：overflow=True 时触发点判读语义已按 R31 修正入档；
