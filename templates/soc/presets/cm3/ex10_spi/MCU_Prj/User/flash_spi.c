...
uint8_t TxBuffer[]="CM3 Flash read and write tests...\r\n";
uint8_t RxBuffer[BufferSize];


SPI_TypeDef* flashSPI= CM3DS_MPS2_SPI0;
...
void SPI_Read_Flash_ID(void)
{
	...
	SPI_WriteByte(flashSPI, W25Q_READ_ID);
...
{	
	SPI_SendData(flashSPI, W25Q_WRITE_EN);// Write Enable
	SPI_Cmd( flashSPI, ENABLE);
...
{	
	...
	do
	{
		...
		SPI_SendData(flashSPI, W25Q_READ_SR0);
		SPI_Cmd( flashSPI, ENABLE);
		SPI_WaitSendFinish(flashSPI);
...
{	
	SPI_SendData(flashSPI, W25Q_SECTOR_ERASE);// 4KB Sector Erase
...
{		
	SPI_WriteByte(flashSPI, W25Q_ALL_ERASE);// Write Enable
	SPI_Cmd( flashSPI, ENABLE);
	SPI_WaitSendFinish(flashSPI);
	SPI_Cmd( flashSPI, DISABLE);
}
...
{	
	...
	SPI_SendData(flashSPI, W25Q_PAGE_PRG);// Page Write
...
void CM3_FLASH_SectorErase(uint32_t startAddr)
{	
	Flash_WriterEnable();
...
void CM3_FLASH_PageWrite(uint8_t* txBuf,uint32_t addr,uint16_t bufSize)
{	
	Flash_WriterEnable();
...
/*Flash页读*/
void CM3_FLASH_PageRead(uint8_t* rxBuf,uint32_t addr,uint16_t num)
{
	int i=0;
	
	SPI_ReceiveDataNum_Config(flashSPI, num);
	SPI_SendData(flashSPI, W25Q_READ);				//Read Data Order  0x03  ×2：0x3b  ×4：0x6b
...
void SPI_Flash_Test(void)
{
	...
	//SPI_LoopBack_Test(CM3DS_MPS2_SPI0);
	...
	/*Flash数据读写*/
	/*Sector擦除*/
	CM3_FLASH_SectorErase(testAddr);
	printf ("CM3_FLASH_SectorErase,addr=0x%x\r\n",testAddr);
	/*按页写入数据*/
	CM3_FLASH_PageWrite(TxBuffer, testAddr,BufferSize);
	printf ("CM3_FLASH_PageWrite:\r\naddr=0x%x,write_data:%s",testAddr,TxBuffer);
	/*按页读取数据*/
	memset(RxBuffer,0x0,sizeof(RxBuffer));
	CM3_FLASH_PageRead(RxBuffer,testAddr,BufferSize);
	printf ("CM3_FLASH_PageRead:\r\naddr=0x0%x,read_data:%s",testAddr, RxBuffer);
}
