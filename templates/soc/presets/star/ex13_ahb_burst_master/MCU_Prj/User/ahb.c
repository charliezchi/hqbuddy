...
void DMA_BURST_WRITE_AHB(void)
{
	...
	DMA_InitStr.DMA_Destination_Addr = 0x60000000;			//AHB总线基地址
	...
	DMA_TransferSize_Config(STAR_DMA,DMA_Channel_0,wr_data_num);
	DMA_Interrupt_Config(STAR_DMA,DMA_Channel_0,DMA_INT_TC,ENABLE);
	DMA_Channel_Enable(STAR_DMA,DMA_Channel_0,ENABLE);		//使能对应通道的DMA 开始传输
...
void DMA_BURST_READ_AHB(void)
{
	...
	/*DMA初始化配置*/
	DMA_InitStr.DMA_Source_Addr = 0x60000000;					//AHB总线基地址
