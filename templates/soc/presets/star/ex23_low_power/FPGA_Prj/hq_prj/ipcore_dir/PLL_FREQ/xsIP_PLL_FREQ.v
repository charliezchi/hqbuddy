// D:\hqui\hqv3_xist_3.0.6_FT061525_win64\build\ipcreator\sup_files\ipdepot\pll\pll_freq_besides_30k\..\_ipgen_seal_\pll_seal.exe -meta_xml D:\hqui\hqv3_xist_3.0.6_FT061525_win64\build\ipcreator\sup_files\ipdepot\pll\pll_freq_besides_30k\pll_freq_25k.xml -ini_file D:/LCZ/work/MCU/demo/XIST_STAR_Demo_2025_1226/ex23_low_power/FPGA_Prj/hq_prj/ipcore_dir/PLL_FREQ/xsIP_PLL_FREQ.hqip -lang chs


`timescale 1 ns / 1 ps
module PLL_FREQ (CLKI, CLKOP, LOCK)/* synthesis NGD_DRC_MASK=1 */;
    input wire CLKI;
    output wire CLKOP;
    output wire LOCK;
    ...
    generate
        if(DEVICE=="SEAL") begin
            defparam u_pll_rst_reg_sync0.INIT = 1'b1; xsDFFSA_K1 u_pll_rst_reg_sync0 (.C(CLKI_t),.D(rst_pre),   .Q(rst_reg[0]));
            defparam u_pll_rst_reg_sync1.INIT = 1'b1; xsDFFSA_K1 u_pll_rst_reg_sync1 (.C(CLKI_t),.D(rst_reg[0]),.Q(rst_reg[1]));
            defparam u_pll_rst_reg_sync2.INIT = 1'b1; xsDFFSA_K1 u_pll_rst_reg_sync2 (.C(CLKI_t),.D(rst_reg[1]),.Q(rst_reg[2]));
            defparam u_pll_rst_reg_sync3.INIT = 1'b1; xsDFFSA_K1 u_pll_rst_reg_sync3 (.C(CLKI_t),.D(rst_reg[2]),.Q(rst_reg[3]));
            defparam u_pll_rst_reg_sync4.INIT = 1'b1; xsDFFSA_K1 u_pll_rst_reg_sync4 (.C(CLKI_t),.D(rst_reg[3]),.Q(rst_reg[4]));
        end
        else begin
            xsDFF_K1G1 u_pll_rst_reg_sync0(.D(rst_pre),   .CK(CLKI_t),.Q(rst_reg[0]));
            xsDFF_K1G1 u_pll_rst_reg_sync1(.D(rst_reg[0]),.CK(CLKI_t),.Q(rst_reg[1]));
            xsDFF_K1G1 u_pll_rst_reg_sync2(.D(rst_reg[1]),.CK(CLKI_t),.Q(rst_reg[2]));
            xsDFF_K1G1 u_pll_rst_reg_sync3(.D(rst_reg[2]),.CK(CLKI_t),.Q(rst_reg[3]));
            xsDFF_K1G1 u_pll_rst_reg_sync4(.D(rst_reg[3]),.CK(CLKI_t),.Q(rst_reg[4]));
        end
    endgenerate
