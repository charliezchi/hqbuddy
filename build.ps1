...
$PROXY_DIR = Join-Path $env:APPDATA "hqbuddy"

function Show-Help {
    Write-Host "Usage: .\build.ps1 [command]"
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  build    Build hqbuddy.exe using PyInstaller"
...
function Invoke-Clean {
    ...
    $items = @(
        @{ Path = "hqbuddy.exe"; Type = "file"; Label = "hqbuddy.exe" },
...
function Invoke-Build {
    ...
    Write-Host "  Building hqbuddy.exe" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    @"
from hqbuddy.__main__ import main
main()
"@ | Set-Content -Path $entryFile -Encoding UTF8

    # Clean previous build
    Write-Host "Cleaning previous build artifacts..."
    $cleanItems = @("dist", "build", "hqbuddy.spec", "hqbuddy.exe")
    ...
    python -m PyInstaller --onefile --name hqbuddy --add-data "scripts;scripts" "$entryFile"
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
    ...
    # Copy to root
    $src = Join-Path $SCRIPT_DIR "dist\hqbuddy.exe"
    $dst = Join-Path $SCRIPT_DIR "hqbuddy.exe"
    Copy-Item -Path $src -Destination $dst -Force
    if ($LASTEXITCODE -ne 0 -and -not (Test-Path $dst)) {
        Write-Host "[FAIL] Failed to copy hqbuddy.exe." -ForegroundColor Red
        if (Test-Path $entryFile) { Remove-Item $entryFile -Force }
        exit 1
    }
    Write-Host "[OK] hqbuddy.exe created." -ForegroundColor Green
    ...
    Remove-Item -Path (Join-Path $SCRIPT_DIR "hqbuddy.spec") -Force -ErrorAction SilentlyContinue
...
function Invoke-Register {
    Write-Host "Registering hqbuddy to user PATH..." -ForegroundColor Cyan
    Write-Host ""

    if (-not (Test-Path (Join-Path $SCRIPT_DIR "hqbuddy.exe"))) {
        Write-Host "[FAIL] hqbuddy.exe not found in project directory." -ForegroundColor Red
    ...
    $src = Join-Path $SCRIPT_DIR "hqbuddy.exe"
    $dst = Join-Path $PROXY_DIR "hqbuddy.exe"
    Copy-Item -Path $src -Destination $dst -Force
    Write-Host "[OK] Copied hqbuddy.exe to proxy directory." -ForegroundColor Green
    ...
    Write-Host ""
    Write-Host "Please restart your terminal to use 'hqbuddy'." -ForegroundColor Cyan
}
